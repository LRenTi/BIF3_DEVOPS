...
In den DNS Server Primary und Secondary muss in Zugriffskontrollliste die IP Adresse des LDAP Servers hinzugefügt werden. Siehe oben im Abschnitt \ref{sec:dns-acces-controll-list}. 
Als Referenz haben wir unsere Änderung dokumentiert:
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{data/2_add_ldap_to_DNS.png}
	\caption{IDAP zu Access Contoll List hinzugefügt}
	\label{fig:IDAP zu Access Contoll List hinzugefügt}
\end{figure}

\subsubsection{Zone Files}
Im Primary DNS Server müssen die Zone Files für die LDAP Domain hinzugefügt werden. Siehe oben im Abschnitt \ref{subsec:forward-zone} und folgende.
Als Referenz haben wir unsere Änderung dokumentiert:
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{data/3_add_ldap_to_zone_files.png}
	\caption{IDAP im DNS Lookup hinzugefügt}
	\label{fig:IDAP im DNS Lookup hinzugefügt}
\end{figure}


\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{data/4_add_ldap_to_zone_files_reverse.png}
	\caption{IDAP im DNS Reverse-Lookup hinzugefügt}
	\label{fig:IDAP im DNS Reverse-Lookup hinzugefügt}
\end{figure}
...
\subsection{LDAP}
Als nächstes richten wir den OpenLDAP-Server ein. Dazu verbinden wir uns über SSH mit der LDAP-EC2-Instanz, aktualisieren alle Systempakete und starten das System neu.
...
Dann installieren wir den LDAP-Server und die Kommandozeilen-Utilities:
...
Als nächstes konfigurieren wir einige Standardwerte, um uns später Tipparbeit zu ersparen.
Führen Sie aus:
...
\subsubsection{Verzeichnis füllen}
...
Erstellen Sie eine neue LDIF-Datei und nennen Sie sie beispielsweise \begin{verbatim}add_content.ldif\end{verbatim}
Der genaue Dateiname ist nicht entscheidend, solange er später korrekt referenziert wird.
Mit dem Kommando \texttt{sudo nano add\_content.ldif} wird die LDIF-Datei für eine Person geändert:


\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{data/suche_1.png}
	\caption{Erstellen einer Person}
	\label{fig:Erstellen einer Person}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{data/suche_2.png}
	\caption{Suchen einer Person}
	\label{fig:Suchen einer Person}
\end{figure}


Nun müssen wir ein gültiges Passwort für den Benutzer Samuel festlegen. Wir haben bisher nur {CRYPT}x als Platzhalter verwendet, um ein Konto ohne Passwort zu erstellen.
(Streng genommen ist {CRYPT}x auch ein gültiges Passwort. Allerdings gibt es keine Eingabe für die Hash-Funktion, die zu der Ausgabe x führt, was bedeutet, dass wir im Grunde ein ungültiges Passwort definiert haben.)
Um das Passwort in ein gültiges zu ändern, können wir ldappasswd verwenden:
...
\subsubsection{LDAP-Server Verbindung testen}

Schließlich können wir testen, ob der LDAP-Server von anderen Instanzen aus erreichbar ist. Wechseln wir zur GitLab-Instanz, installieren LDAP-Utils und führen aus:
...
\subsection{Integration von LDAP mit GitLab}

Als nächstes werden wir LDAP mit GitLab integrieren. Dazu folgen wir der offiziellen Dokumentation.
Zunächst müssen wir einige Einstellungen konfigurieren, die wir über Umgebungsvariablen in Docker vornehmen. Bearbeiten wir also erneut die docker-compose.yml unseres LDAP-Servers:

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{data/dockerfile.png}
	\caption{Gitlab Yaml erstellen}
	\label{fig:Gitlab Yaml erstellen}
\end{figure}


Die genaue Wirkung all dieser Einstellungen ist in der oben verlinkten Dokumentation gut beschrieben.
Benutzer können sich nun mit ihrer LDAP-UID und ihrem Passwort anmelden.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{data/5_gitlab_mit_ldap.png}
	\caption{Gitlab mit LDAP}
	\label{fig:Gitlab mit LDAP}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{data/6_login_mit_ldap_user_profile_page.png}
	\caption{LDAP User Ansicht in GitLab}
	\label{fig:LDAP User Ansicht in GitLab}
\end{figure}
\section{Kosten Kalkualtion}
Wir haben insgesamt 5 EC2 Instanzen erstellt. Davon sind 4 Instanzen t2.micro und 1 Instanz t3.large. Laut dem Cost Calcuator von AWS
kommen wir auf folgende Kosten: \\ \\
Monatliche Kosten:  31.98 USD \\
Jährliche Kosten: 383.76 USD  \\

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{data/cost_calculator.png}
	\caption{Kosten Kalkulation}
	\label{fig:cost_calculator}
\end{figure}
